...
///
/// Deliberately separate from <see cref="IPreferenceSettings{TSelf}"/>: how preferences are
/// stored and how they are presented are not the same grouping. The print bed record backs both
/// the Print Bed and Air Channels pages, and the cut/split record backs both Cut and Split, so
/// seven settings records supply nine pages. A feature contributes as many pages as it needs,
/// over as many settings records.
/// </summary>
public interface IPreferencePage {
    ...
    /// cannot push General or Print Bed down the list.
    /// </summary>
    int Order { get; }
...
public static class PreferencePageExtensions {
    public static bool Matches(this IPreferencePage section, string? search) {
