...
/// </summary>
public class MeshOffsetSignedDistanceGrid {
    public DMesh3 Mesh;
    public DMeshAABBTree3 Spatial;
    public float CellSize;

    // Distance multiplier for each triangle for the offsets having an uneven application.
    public float[] WeightedOffsets = []; 
    ...
    public MeshOffsetSignedDistanceGrid(DMesh3 mesh, double cellSize, float[] weightedOffsets, DMeshAABBTree3 spatial = null) {
        Mesh = mesh;
        CellSize = (float)cellSize;
        Spatial = spatial;
        WeightedOffsets = weightedOffsets;
    }


    public void Compute() {
        // check if offsets are valid
        if (WeightedOffsets.Length != Mesh.TriangleCount)
            throw new Exception("MeshSignedDistanceGrid.Compute: WeightedOffsets must have same length as Mesh.TriangleCount");
